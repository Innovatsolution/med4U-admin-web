...
function RecentOrders({tripReload}) {
  const [trips, setRoles] = useState([]);
  const [loading, setLoading] = useState(true);
    ...
    const fetchRoles = async () => {
      ...
      try {
        const data = await getRequest('/trips');
        setRoles(data);
      } catch (error) {
  ...
  return (
    <Card>
      {loading ? (
        ...
        <RecentOrdersTable trips={trips}  onTripRefresh={handleRefresh}/>
      )}
      </Card>
  );
}
